...
export class BankrunConnection {
  ...
  private async updateSlotAndClock() {
    ...
    const nextSlot = currentSlot + BigInt(1); // Sometime throw the slot not in future error, try to increase this vaule
  ...
  onLogs(filter: LogsFilter, callback: LogsCallback, _commitment?: Commitment) {
  ...
  async removeOnLogsListener(clientSubscriptionId): Promise<void> {
  ...
  async removeAccountChangeListener(clientSubscriptionId): Promise<void> {
